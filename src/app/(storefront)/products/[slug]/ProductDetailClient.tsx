  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (productsIdx !== -1 && pathParts[productsIdx + 1]) {
        clientSlug = decodeURIComponent(pathParts[productsIdx + 1]);
      }
    }

    if (!clientSlug || clientSlug === '_') {
      setError('Product not found.');
      setLoading(false);
      return;
    }

    if (lastFetchedRef.current === clientSlug) {
      return;
    }
    lastFetchedRef.current = clientSlug;

    async function loadData() {
      setLoading(true);
      setError('');
      try {
        const prodRes = await apiClient.get(`/products/${clientSlug}`);
